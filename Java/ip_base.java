...
public class ip_base {
    private double[] w, p;
    private double cap, mejorP = 0;
    private int n;
    public long nodosVisitados = 0;

    public ip_base(double[] weights, double[] profits, double capacity) {
        this.w = weights; this.p = profits; this.cap = capacity; this.n = weights.length;
    }

    private void backtrack(int nivel, double pesoA, double benefA) {
        nodosVisitados++;
    ...
    public static void main(String[] args) {
        ...
        for (int n_actual : n_pruebas) {
            ...
            ip_base solver = new ip_base(pesos, beneficios, sumW * 0.5);
            solver.resolver();
        }
    }
}